...
;;; $Id: windows.el,v 2.39 2003/03/28 11:34:41 yuuji Exp yuuji $
;;; (c) 1993-2002 by HIROSE Yuuji [yuuji@gentei.org]
;;; Last modified Thu Jun 24 16:15:35 2004 on firestorm
...
;;;	一さん、メニューによるウィンドウ切り替えを提案してくださった(株) 
;;;	東芝の小林勉さん、XEmacs関係の動作報告などを頂いた中島幹夫さん、
;;;	各ウィンドウに名前をつける機能などのパッチを頂いた早稲田大学の西
;;;	本さんに感謝致します。
;;;
;;;【取り扱い】
;;;
...
(defun win:memorize-current-conf-1 ()
  ...
  (win:adjust-window t)
  (aset win:memorized-winconf win:current-config (current-window-configuration)))
...
selected window.  Without ARG, selected window and its point reserved."
  ...
  (let ((memorized (aref win:memorized-winconf win:current-config))
	...
	(message "No conf. yet.  Modify this buffer, and you get conf.")
      ...
      (set-window-configuration memorized)
      (if (equal 1 arg)
	  (progn
	    (select-window sw)
